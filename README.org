...
Peristence will be removed.  Chat logs are basically no more beneficial than process output.  We use it to debug, but ultimately the output is what we are working on, not the chat that was used to create it.
